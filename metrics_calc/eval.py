...
from loader.dataloader_functions import scale_mri
...
def test():
    main_path = '/raid/data/DA_BrainDataset/'
    df = create_df(main_path + 'images/', main_path + 'masks/')
    name_values = ['sie3_to_ge15', 'ge3_to_ph3', 'ph3_to_ge15', 'sie15_to_ph3', 'ph3_to_sie15', 'ge15_to_ph15']

    gan_values = ['ge15_to_sm3', 'ph3_to_ge3', 'ge15_to_ph3', 'ph3_to_sm15', 'sm15_to_ph3', 'ph15_to_ge15']
    data_directories = [('siemens_3', 'ge_15'), ('ge_3', 'philips_15'), ('philips_3', 'ge_15'), ('siemens_15', 'philips_3'),
                        ('philips_3', 'siemens_15'), ('ge_15', 'philips_15')]
    ...
    # path_json = f'/raid/data/DA_BrainDataset/split_settings/{name_values}/'
    for name, (test_domain, learned_domain), weight_p, weight_o, gan_dir in zip(name_values, data_directories, weights_names_predict, weigths_names_own, gan_values):
        ...
        for json_file, is_column_val in zip([json_val, json_train], [True, False]):
            print(len(json_file))
            for id in json_file:
                gan_file_style = find_gan_file(gan_path_style, gan_dir, id)
                gan_file_cycle = find_gan_file(gan_path_cycle, gan_dir, id)
                row = df.loc[id]
                print(row)
                path, mask = row.image_path, row.mask_path
                ...
                current, baseline, styled, st_gan, cy_gan = basic_eval([net, net_own], path, mask, path_donor, [gan_file_style, gan_file_cycle])

                result.append((id, short_name_path, short_name_donor, current[0], current[1], baseline[0], styled[0], baseline[1], styled[1],
                ...
                df_result = pd.DataFrame(result, columns=['id', 'target', 'donor', 'current_sdice', 'current dice',
                                                          ...
                                                          'styled_dice', 'StyleGan_sdice', 'StyleGan_dice','CycleGan_sdice',

...
class SoloClassDiceLossIvan(torch.nn.Module):
    def __init__(self, n_classes = 1):
        super(SoloClassDiceLossIvan, self).__init__()

        self.softmax = torch.nn.Softmax(dim=1)
        self.n_classes = n_classes
    def forward(self, pred, target):
        pred = pred.detach().cpu().numpy().squeeze().astype(bool)
        target = target.detach().cpu().numpy().squeeze().astype(bool)
